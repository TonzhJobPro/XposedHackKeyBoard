...
public class RunNext
{
            ...
            throws Throwable
    {
        ...
        executeInput(String.format("adb -s %s shell rm /data/local/tmp/next_output", new Object[] { deviceId })).waitFor();
        while (true)
        {
            Runtime.getRuntime().exec(String.format("adb -s %s push %s /sdcard/next_input", new Object[] { deviceId, inFilePath }))
                    .waitFor();
            ...
            File tmpInput = File.createTempFile("runnext-", "-part");
            try {
                BufferedWriter bw = new BufferedWriter(new FileWriter(tmpInput));
                try {
                    bw.write(String.format("do_next /sdcard/next_input /data/local/tmp/next_output %s", new Object[] { languageCode }));
                } finally {
                    bw.close();
                }
            ...
            while ((line = rd.readLine()) != null) {
                ...
                if (line.contains("delete file")) {
                    ...
                    executeInput(String.format("adb -s %s shell rm /sdcard/next_input", new Object[]{deviceId}));
                }
            }
            logcat.destroy();
