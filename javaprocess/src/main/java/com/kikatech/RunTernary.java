...
public class RunTernary
{
            ...
            throws Throwable
    {
        ...
        executeInput(String.format("adb -s %s shell rm /data/local/tmp/ternary_output", new Object[] { deviceId })).waitFor();
        while (true)
        {
            Runtime.getRuntime().exec(String.format("adb -s %s push %s /sdcard/ternary_input", new Object[] { deviceId, inFilePath }))
                    .waitFor();
            ...
            File tmpInput = File.createTempFile("runternary-", "-part");
            try {
                BufferedWriter bw = new BufferedWriter(new FileWriter(tmpInput));
                try {
                    bw.write(String.format("do_ternarynew /sdcard/ternary_input /data/local/tmp/ternary_output %s", new Object[] { languageCode }));
                } finally {
                    bw.close();
                }
            ...
            while ((line = rd.readLine()) != null) {
                ...
                if (line.contains("delete file")) {
                    ...
                    executeInput(String.format("adb -s %s shell rm /sdcard/ternary_input", new Object[]{deviceId}));
                }
            }
            logcat.destroy();
